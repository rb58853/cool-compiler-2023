(* hairy  . . .*)
class Bazz inherits IO {

     h : Int <- 1;

     g : Bazz  <- case self of
		     	n : Bazz => {out_string("Bazz => Bazz\n");(new Foo);};
		     	n : Razz => {out_string("Bazz => Razz\n");(new Bar);};
			     n : Foo  => {out_string("Bazz => Foo \n");(new Razz);};
			     n : Bar =>  {out_string("Bazz => Bar \n"); self;};
		  esac;

     i : Object <- printh();

     printh() : Int { { out_int(h); 0; } };

     doh() : Int { (let i: Int <- h in { h <- h + 1; i; } ) };
};

class Foo inherits Bazz {
     a : Foo <- case self of
		      n : Razz => {out_string("Foo => Razz\n");(new Bar);};
		      n : Foo =>  {out_string("Foo => Foo\n");(new Razz);};
		      n : Bar =>  {out_string("Foo => Bar\n");self;};
   	         esac;

     b : Int <- a.doh()+ g.doh() + doh() + printh();

     doh() : Int { (let i : Int <- h in { h <- h + 2; i; } ) };

};

class Razz inherits Foo {

     e : Razz <- case self of
		  n : Razz => {out_string("Razz => Razz\n");(new Bar);};
		  n : Bar => {out_string("Razz => Bar  \n");self;};
		esac;

     f : Int <- a@Bazz.doh() + g.doh() + e.doh() + doh() + printh();

};

class Bar inherits Razz {

     c : Int <- doh();

     d : Object <- printh();
};



(* scary . . . *)
class Main {
    a : Bazz <- new Bazz;
    b : Foo <- new Foo;
    c : Razz <- new Razz;
    d : Bar <- new Bar;

  main(): String { "do nothing" };

};





